...
public class UsuarioController {
    ...
    public ResponseEntity<Usuario> listarPorIdUsuario(@PathVariable long id_usuario){
        try {
            ...
            return usuarioOpt.map(equipo -> new ResponseEntity<>(equipo, HttpStatus.OK))
    ...
    public ResponseEntity<Usuario> actualizarUsuario(@PathVariable long id_equipo, @RequestBody Equipo equipoActualizado){
        try {
            Equipo equipo = equipoService.actualizarEquipo(id_equipo, equipoActualizado);
            return new ResponseEntity<>(equipo, HttpStatus.OK); // 200 OK
        } catch (NoSuchElementException e) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND); // 404 Not Found
        } catch (IllegalArgumentException e) {
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST); // 400 Bad Request
